...
| ADR-016 | Require parent library membership for season and episode tracking. | Accepted | Personal progress must not exist independently from the user's relationship with its parent series. | `SeasonEntry` and `EpisodeProgress` require a `LibraryEntry` for the same user and parent series; creation is ordered or transactional. |
| ADR-017 | Require library membership for favorites. | Accepted | Favorites are a curated subset of a user's library, not an independent catalog list. | A `Favorite` requires the same user's matching `LibraryEntry` and is removed with that entry. |
| ADR-018 | Store explicit ownership throughout the private manual media hierarchy. | Accepted | Inherited-only ownership is too easy to omit from queries and weakens cross-user isolation. | Private manual media items, seasons, and episodes carry matching owner scope enforced by the database and domain authorization. |
| ADR-019 | Do not attach provider mappings to private manual media in v1. | Accepted | Global provider uniqueness must not collide with or expose private user-created catalog data. | Provider mappings target shared catalog media only; manual promotion, merging, and deduplication remain deferred. |
| ADR-020 | Restrict v1 activity targets to durable catalog entities. | Accepted | Events should survive ordinary changes to temporary personal records without weak polymorphic references. | Activity targets only media items, seasons, or episodes; event type and minimal metadata describe the personal action. |
| ADR-021 | Separate user-controlled status from episode-derived progress. | Accepted | Workflow intent and observed granular progress can legitimately differ. | Status totals use explicit `LibraryEntry` state; completion percentages derive from episodes; v1 never silently reconciles status. |
| ADR-022 | Use transactional mutations with last-committed-write behavior in v1. | Accepted | Personal writes need predictable atomicity without premature locking complexity. | Bulk progress uses transactional per-episode upserts, APIs return canonical state, and overlapping writes resolve to the last committed write. |
